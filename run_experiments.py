...
if __name__ == '__main__':
    ...
    parser.add_argument('--level', type=int, action='store', required=True, help='Level of data scarcity [0-3]')
    ...
    parser.add_argument('--dump_test_preds', action='store_true', help='Dump predictions on test data')
    ...
    if args.level == 2:
        ...
        # For limited labeled data setting, target data used for training must also be considered IV
        for example in target_dataset.train_data + target_dataset.dev_data:
            source_vocab.update(example['tokens'])
    ...
    dump_test_preds = True if args.dump_test_preds else False
    if args.level == 0:
        trainer = ZeroShotTrainer(args.lr, args.epochs, device, args.out_dir)
        # trainer.train(model, source_train_batches, source_dev_batches, args.out_dir, source_dataset.label_vocab, source_vocab)
        model.load_state_dict(torch.load(os.path.join(args.out_dir, 'best_model.pt')))
        print('Scores on source testing data')
        # trainer.test(model, source_test_batches, source_dataset.label_vocab, source_oov_indicators)
        print('Scores on target testing data')
        trainer.test(model, target_test_batches, source_dataset.label_vocab, target_oov_indicators, \
                dump_test_preds = dump_test_preds, tokenized_examples = target_dataset.tokenized_examples[-1])
    ...
    if args.level == 1:
        ...
        if args.da_method == 'LA':
            ...
            trainer = AdversarialTrainer(args.lr, args.epochs, device, args.out_dir)
            # trainer.train(adv_model, source_train_batches, source_dev_batches, domain_batches, args.out_dir, source_dataset.label_vocab, source_vocab)
            adv_model.load_state_dict(torch.load(os.path.join(args.out_dir, 'best_model.pt')))
            print('Scores on source testing data')
            # trainer.test(adv_model, source_test_batches, source_dataset.label_vocab, source_oov_indicators)
            print('Scores on target testing data')
            trainer.test(adv_model, target_test_batches, source_dataset.label_vocab, target_oov_indicators, \
                    dump_test_preds = dump_test_preds, tokenized_examples = target_dataset.tokenized_examples[-1])
        if args.da_method == 'IW':
            ...
            trainer = InstanceWeightingTrainer(args.lr, args.epochs, device, args.out_dir)
            # trainer.train(iw_model, source_train_batches, source_dev_batches, domain_batches, args.out_dir, source_dataset.label_vocab, source_vocab)
            iw_model.load_state_dict(torch.load(os.path.join(args.out_dir, 'best_model.pt')))
            print('Scores on source testing data')
            # trainer.test(iw_model, source_test_batches, source_dataset.label_vocab, source_oov_indicators)
            print('Scores on target testing data')
            trainer.test(iw_model, target_test_batches, source_dataset.label_vocab, target_oov_indicators, \
                    dump_test_preds = dump_test_preds, tokenized_examples = target_dataset.tokenized_examples[-1])
        if args.da_method == 'PL':
            ...
            trainer = SelfTrainer(args.lr, args.epochs, device, args.pl_passes, args.out_dir)
            # trainer.train(model, source_train_batches, source_dev_batches, unlabeled_batches, args.out_dir, source_dataset.label_vocab, source_vocab, args.batch_size)
    ...
    if args.level == 2:
        if args.da_method == 'TGONLY':
            trainer = ZeroShotTrainer(args.lr, args.epochs, device, args.out_dir)
            # trainer.train(model, target_train_batches, target_dev_batches, args.out_dir, source_dataset.label_vocab, source_vocab)
            model.load_state_dict(torch.load(os.path.join(args.out_dir, 'best_model.pt')))
            print('Scores on source testing data')
            # trainer.test(model, source_test_batches, source_dataset.label_vocab, source_oov_indicators)
            print('Scores on target testing data')
        ...
        if args.da_method == 'SCTGJL':
            ...
            # trainer.train(model, joint_train_batches, joint_dev_batches, args.out_dir, source_dataset.label_vocab, source_vocab)
            model.load_state_dict(torch.load(os.path.join(args.out_dir, 'best_model.pt')))
            print('Scores on source testing data')
            # trainer.test(model, source_test_batches, source_dataset.label_vocab, source_oov_indicators)
            print('Scores on target testing data')
        ...
        if args.da_method == 'FA':
            ...
            # trainer.train(model, joint_train_batches, joint_dev_batches, args.out_dir, source_dataset.label_vocab, joint_train_domains, joint_dev_domains, source_vocab)
            model.load_state_dict(torch.load(os.path.join(args.out_dir, 'best_model.pt')))
            print('Scores on source testing data')
            # trainer.test(model, source_test_batches, source_dataset.label_vocab, [0]*len(source_test_batches), source_oov_indicators)
            print('Scores on target testing data')
            trainer.test(model, target_test_batches, source_dataset.label_vocab, [1]*len(target_test_batches), target_oov_indicators, \
                    dump_test_preds = dump_test_preds, tokenized_examples = target_dataset.tokenized_examples[-1])
        if args.da_method == 'LA':
        ...
        if args.da_method == 'IW':
            ...
            #        args.out_dir, source_dataset.label_vocab, source_vocab, args.batch_size)
            model.load_state_dict(torch.load(os.path.join(args.out_dir, 'best_model.pt')))
            print('Scores on source testing data')
            # trainer.test(model, source_test_batches, source_dataset.label_vocab, source_oov_indicators)
            print('Scores on target testing data')
